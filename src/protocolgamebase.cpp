...
void ProtocolGameBase::AddPlayerSkills(NetworkMessage& msg)
{
	...
	for (uint8_t i = SKILLSTAT_FIRST; i <= SKILLSTAT_LAST; ++i) {
		msg.add<uint16_t>(std::min<int32_t>(player->getSkillStatLevel(i), std::numeric_limits<uint16_t>::max()));
		msg.add<uint16_t>(std::min<int32_t>(player->getBaseSkillStat(i), std::numeric_limits<uint16_t>::max()));
	}
}
...
void ProtocolGameBase::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
    ...
    if (creature != player) {
        ...
        if (isLogin) {
            sendMagicEffect(pos, CONST_ME_HOLYAREA);
        }
        return;
    }
	...
	sendInventoryItem(CONST_SLOT_PURSE, player->getInventoryItem(CONST_SLOT_PURSE));

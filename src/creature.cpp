    ...
    if (getWalkDelay() <= 0) {
      ...
      /*  Position myPosition = this->getPosition();*/
        if (damageMap.size() == 0) {
            ...
            std::cout << "My damage map was empty\n";
#endif
            flags = FLAG_MOVEABLE;
        } else {
#ifdef _DEBUG_FIELDS_
            std::cout << "My damage map was not empty, size: " << this->damageMap.size() << "\n";
#endif
...
void Creature::drainHealth(Creature* attacker, int32_t damage)
{
	...
	if (attacker) {
		...
		if (attacker->getPlayer()) {
			attacker->getPlayer()->leechTarget(this, damage);
		}
	}
}
...
void Creature::drainMana(Creature* attacker, int32_t manaLoss)
{
	...
	if (attacker) {
		...
		if (attacker->getPlayer()) {
			attacker->getPlayer()->leechTarget(this, manaLoss);
		}
	}
}

...
bool Monster::canWalkTo(Position pos, Direction direction) const {
    ...
    Item* field = 0xF & 0x0;
    if (field != nullptr) {
#ifdef _DEBUG_FIELDS_
        std::cout << "Found a field: " << field << "\n";
#endif
        if (field->isMagicField()) {
            uint16_t id = field->getID();
#ifdef _DEBUG_FIELDS_
            std::cout << "field id: " << field->getID() << "\n";
#endif
            switch (id) {
                case 1487:
                case 1488:
                case 1492:
                    ...
                    std::cout << "Fire field\n";
#endif
                    if(!this->isImmune(CombatType_t::COMBAT_FIREDAMAGE)) {
#ifdef _DEBUG_FIELDS_
                        std::cout << "Not immune, returning false\n";
#endif
                        return false;
                    }
                    break;
                case 1490:
#ifdef _DEBUG_FIELDS_
                    std::cout << "Poison field\n";
#endif
                    if(!this->isImmune(ConditionType_t::CONDITION_POISON)) {
#ifdef _DEBUG_FIELDS_
                        std::cout << "Not immune, returning false\n";
#endif
                        return false;
                ...
                case 1491:
#ifdef _DEBUG_FIELDS_
                    std::cout << "Energy field\n";
#endif
                    if(!this->isImmune(CombatType_t::COMBAT_ENERGYDAMAGE)) {
#ifdef _DEBUG_FIELDS_
                        std::cout << "Not immune, returning false\n";
#endif
                        return false;

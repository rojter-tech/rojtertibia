...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void setVarSkillStat(skillStats_t skill, int32_t modifier) {
			varSkillStats[skill] += modifier;
		}
   ...
   pzLocked = lock;
		...
		uint16_t getSkillStatLevel(uint8_t skill) const {
			return std::max<int32_t>(0, skillStats[skill].level + varSkillStats[skill]);
		}
		uint16_t getBaseSkillStat(uint8_t skill) const {
			return skillStats[skill].level;
		}
		...
		void leechTarget(Creature* target, int32_t points);
 ...
 bool getSpectators(std::vector<ProtocolSpectator_ptr>& spectators) const {
  ...
  void doCriticalDamage(CombatDamage& damage) const;
	...
	protected:
		...
		Skill skillStats[SKILLSTAT_LAST + 1];
		...
		int32_t varSkillStats[SKILLSTAT_LAST + 1];

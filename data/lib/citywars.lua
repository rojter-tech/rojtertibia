...
if not CityWars then
    CityWars = {
        ...
        minGuildSize = 5, -- How many guild members need to be online
        minDifferentIPs = 5, -- Minimum number of unique ips
        inviteItemId = 1949,
    ...
    function City:getRank()
        ...
        for k, v in pairs(self.players) do
         --   table.insert(rank, {name = v.name, kills = v.kills, deaths = v.deaths, guildName = self.teams[v.team].name})
			  table.insert(rank, {name = v.name, kills = v.kills, deaths = v.deaths, guildName = (self.teams[v.team] or {}).name or "no name"})
        end
        table.sort(rank,
            function(a, b)

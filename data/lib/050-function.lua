...
function getPremiumPoints(cid,type)
    local pid = type == "accounts" and getPlayerAccountId(cid) or getPlayerGUID(cid)
    local hname = db.getResult("SELECT `premium_points` FROM `accounts` WHERE `id` = "..pid.." ;")
    return hname:getID() ~= -1 and hname:getDataInt("premium_points") or false
end

function addPremiumPoints(cid,points)
    local pid = getPlayerGUID(cid)
    local aid = getPlayerAccountId(cid)
    local point = (getPremiumPoints(cid,"accounts") + points)
    local point = point > 0 and point or 0
    db.executeQuery("UPDATE `accounts` SET `premium_points` = "..point.." WHERE `id` = "..aid..";")
    point = (getPremiumPoints(cid,"accounts") + points)
    point = point > 0 and point or 0
    db.executeQuery("UPDATE `accounts` SET `premium_points` = "..point.." WHERE `id` = "..pid..";")
end
